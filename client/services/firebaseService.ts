...
export class FirebaseService {
  ...
  static subscribeToItems(callback: (items: LostFoundItem[]) => void): () => void {
    const q = query(
      collection(db, COLLECTION_NAME), 
      orderBy('createdAt', 'desc')
    );
  ...
  static async getRecentItems(limitCount: number = 6): Promise<LostFoundItem[]> {
    try {
      const q = query(
        collection(db, COLLECTION_NAME), 
        orderBy('createdAt', 'desc'),
  ...
  // Get items by status
  static async getItemsByStatus(status: 'lost' | 'found'): Promise<LostFoundItem[]> {
    try {
      const q = query(
        collection(db, COLLECTION_NAME), 
        where('status', '==', status),
  ...
  // Get statistics
  static async getStats(): Promise<{totalItems: number, lostItems: number, foundItems: number}> {
    try {
      const allItems = await this.getAllItems();
